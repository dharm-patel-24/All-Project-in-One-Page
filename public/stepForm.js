...
function educationValidation(eduID) {
    for (i = 0; i < eduID.length; i++) {
        var percent = /^(100(.0{1,2})?|([0-9]?0-9))$/;
            ...
            if (!(str)) {
                document.getElementsByClassName('error')[i].innerHTML = eduID[i] + " should not be empty";
                // document.getElementById('lasterror').innerHTML = ID[i] + " should not be empty";
                return str;
                ...
                if (!(str.match(/^[0-9]+$/)) || str.length != 4) {
                    document.getElementsByClassName('error')[i].innerHTML = eduID[i] + " should be a number with 4 digits";
                    return (str.match(/^[0-9]+$/));
                } else {
                    document.getElementsByClassName('error')[i].innerHTML = "";
                }
                ...
                if (!(str.match(/^(100(\.0{1,2})?|([0-9]?[0-9](\.[0-9]{1,2})))$/))) {
                    document.getElementsByClassName('error')[i].innerHTML = eduID[i] + " should be a number with float";
